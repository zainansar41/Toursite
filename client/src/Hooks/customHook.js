...
export async function AcceptRejectHotel(id, action) {
    try {
        const token = localStorage.getItem('token')
        const { data: { msg }, status } = await axios.post(`/api/AcceptRejectHotel/${id}`, { action }, { headers: { "authorization": `Bearer ${token}` } })
